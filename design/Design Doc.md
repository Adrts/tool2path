...
- **删除声明：** 从配置文件移除该行——
  ...
  - 若**在 PATH 中**弹确认「该目录目前在 PATH 中，删除声明将同时从 PATH 移出（生成待应用变更）」→ 确认后一键“移出并删除声明”。若在应用前退出，该路径仍在 PATH 但已不在配置文件（变回锚点，锁定）；
...
- 报备表单仍只做软校验：缺失标黄但允许保存（可后续重建或重设路径）。
...
| × 删除声明 | 左栏行内 | 从配置文件删除该行；在 PATH 中时确认后“移出并删除声明” | 仅报备库条目 |
