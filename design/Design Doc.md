...
- **启动时读取**配置文件，读不出来则弹窗提示「配置文件读取失败」并退出程序（不做任何内存态改写，绝不用空清单覆盖报备库）。
...
- 配置文件读不出来时（启动、刷新、重新载入一律相同）：弹窗提示「配置文件读取失败」并退出程序，不做任何内存态改写（不存在“空报备库”的中间态）。
- 同目录重复报备提示（R6）。
...
```mermaid
flowchart TD
  A[读取配置文件 registered-paths.yaml] --> B{文件可读?}
  B -- 否 --> B1[弹窗提示配置文件读取失败 并退出程序]
...
| 编码 / 格式 | UTF-8；YAML list（每行 `- path`）；容错解析（跳过注释、空行与 `---` 分隔线；同一路径重复出现时读取即去重并报告行号） |
...
| 边界场景 | 规则冲突点 | 既定行为 |
| --- | --- | --- |
| 配置文件不存在 | R0 | 缺失时自动创建空清单（含头部注释） |
| 配置文件读不出来（权限 / 被占用 / 网络盘断开） | R0 | 启动、刷新、重新载入一律弹窗提示「配置文件读取失败」并退出程序，不改动任何内存态 |
...
| 配置文件读不出来 | R0 | 弹窗提示并退出程序；不做任何内存态改写，不存在“空报备库”的中间态 |
...
| 风险 | 影响 | 缓解措施 |
| --- | --- | --- |
| 用户手工改坏配置文件 | 报备库不可用 | 读不出来时弹窗提示并退出程序，格式不符的行由宽容解析跳过；不生成备份文件，建议把 `registered-paths.yaml` 纳入版本管理（Git / dotfiles） |
...
| 失效项长期无人处理 | 死条目累积 | 状态栏计数（>0 时显示）、行内黄标可见；不做忽略机制，因此不会被静默掩盖 |
...
### 15.1 已实现（自测全通过，当前 48 项）

| 能力 | 对应规则 |
| --- | --- |
| 报备库 / 配置文件读写、外部改动提示、读不出来即弹窗退出 | R0 |
| 报备即接管；未报备路径只读 | R1 |
| 落盘前自动断言锚点相对顺序 | R2 |
...
| 配置文件读不出来时的行为 | 弹窗提示「配置文件读取失败」并退出程序，不改动任何内存态 |
